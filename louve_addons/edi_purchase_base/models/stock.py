...
class StockPicking(models.Model):
    ...
    def remove_file(self, ftp, name, edi_system):
        try:
            ...
            raise ValidationError(_('Error when removing file from ftp server : %s') % tools.ustr(e))
    ...
    def read_stock_picking_file(self, lines, edi_system):
        ...
        if not lines:
            raise ValidationError(_('Please configure fields mapping for BLE interface on your EDI system!'))
        ...
        for line in lines:
            ...
            if line[0] == edi_system.header_code and not delivery_date:
                #Header processing
                delivery_date_mapp = edi_system.ble_mapping_ids.filtered(lambda rec: rec.mapping_field_id.name == 'date_planned')
                ...
                code_mapping = edi_system.ble_mapping_ids.filtered(lambda rec: rec.mapping_field_id.name == 'product_code')
                ...
                if not picking_order:
                    supp_info = product_supp_info.search([('product_code', '=', product_code)], limit=1)
                    ...
                    if supplier_ids:
                        cr = self.env.cr
                        cr.execute('select * from purchase_order where '
                                   'cast (date_planned as date) = %s and partner_id in %s limit 1', (delivery_date,
                ...
                qty_mapping = edi_system.ble_mapping_ids.filtered(lambda rec: rec.mapping_field_id.name == 'product_qty')
                ...
                ordered_product_id = self.env['product.product'].search([('product_tmpl_id', '=', product_tmpl_id.id)],
                                                                        limit=1)
                cr = self.env.cr
                cr.execute('select * from stock_pack_operation where picking_id=%s and product_id=%s limit 1',
                                                                    (picking_order.id, ordered_product_id.id))
                res_stock = cr.fetchone()
                res_id = res_stock and res_stock[0] or False
                ordered_operation= self.env['stock.pack.operation'].browse(res_id)
                ...
                if ordered_quantity != float(product_qty_pack):
                    vals = dict()
                    vals.update({'line_to_update_id': ordered_operation.id, 'product_id': ordered_product_id.id, 'ordered_quantity': ordered_quantity,
                                 'product_qty': float(product_qty_pack), 'package_qty': ordered_operation.package_qty})
                    values_list += [(0, 0, vals)]
        if delivery_sign == '+':
            proposition_vals.update({'name': picking_order.id, 'values_proposed_ids': values_list})
            _logger.info(">>>>>>>>>>>>>>>>>> Creating delivery order update propositions >>>>>>>>>>>>>>>>>>>>>")
    ...
    def cron_update_stock_picking(self):
        ...
        for edi_system in edi_systems_list:
            ...
            # Pull
            line_picking, file_name = ecs_obj.ftp_connection_pull_ble(ftp, edi_system, distant_folder_path, local_folder_path)
            ...
            # Log
            self.env['purchase.edi.log'].create_log_history(_('BLE interface'), edi_system.id)
            # Close FTP
