...
class ProductProduct(models.Model):
    ...
    def read_prices_file(self, lines, edi_system):
        ...
        if not lines:
            raise ValidationError(_('Please configure fields mapping for prices interface on your EDI system!'))
        _logger.info(">>>>>>>>>>>>>>>>>> Reading supplier prices file >>>>>>>>>>>>>>>>>>>>>")
        prices = []
        value = []
        today = datetime.date.today()
        for line in lines:
            # check if this line is already imported.
            product_code_mapp = edi_system.price_mapping_ids.filtered(lambda rec:
                                                                      rec.mapping_field_id.name == 'supplier_code')
            ...
            for mapping in edi_system.price_mapping_ids:
                ...
                if mapping.mapping_field_id.name == 'supplier_code':
                    ...
                    supp_info = product_supp_info.search([('product_code', '=', data)], limit=1)
        ...
        _logger.info(">>>>>>>>>>>>>>>>>> Creating supplier prices >>>>>>>>>>>>>>>>>>>>>>>>>>>")
    ...
    def cron_update_purchase_prices(self):
        ...
        for edi_system in edi_systems_list:
            ...
            # Pull
            line_prices, file_name = ecs_obj.ftp_connection_pull_prices(ftp, distant_folder_path, local_folder_path, edi_system.fnmatch_filter)
            # File Treatment and delete file
            if self.read_prices_file(line_prices, edi_system):
                self.remove_file(ftp, file_name, edi_system)
            # Log
            self.env['purchase.edi.log'].create_log_history(_('Prices interface'), edi_system.id)
            # Close FTP
