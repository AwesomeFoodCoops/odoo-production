...
class ProductTemplate(models.Model):
    ...
    def _compute_pricetag_rackinfos(self):
        for pt in self:
            ...
            if pt.rack_location:
                tmp = pt.rack_location + (" - " + tmp if tmp else "")
            if pt.rack_number_of_packages:
                tmp = pt.rack_number_of_packages + (" - " + tmp if tmp else "")
    ...
    def _compute_pricetag_origin(self):
        for pt in self:
            ...
            if pt.department_id:
                tmp = pt.department_id.name + (" - " + tmp if tmp else "")
            if pt.country_id:
                tmp = pt.country_id.name.upper() + (" - " + tmp if tmp else "")
    ...
    def _check_origin_department_country(self):
        for pt in self:
            if (
                pt.department_id.country_id
                and pt.department_id.country_id.id != pt.country_id.id
            ):
                raise UserError(
