...
class ShiftLeave(models.Model):
    ...
    def _onchange_start_date(self):
        ...
        if self.is_parental_leave:
            ...
            if self.start_date and self.expected_birthdate:
                if expected_birthdate >= today:
                    regular_stop_date = start_date + relativedelta(years=1)
                    ...
                    baby_age = today.year - expected_birthdate.year - \
                        (
                    ...
                    if not baby_age:
                        regular_stop_date = \
                            expected_birthdate + relativedelta(years=1)
                        self.regular_stop_date = regular_stop_date
                        self.stop_date = regular_stop_date
                    else:
                        raise UserError(_(
    ...
    def send_validated_parental_leave_email(self):
        ...
        if self.is_parental_leave:
            ...
            expected_dt = fields.Date.from_string(self.expected_birthdate)
            today_dt = fields.Date.from_string(fields.Date.today())
            if expected_dt < start_dt and expected_dt < today_dt:
